...
module.exports.createQueue = dbClient => ({
  init: async () => {
    ...
    await dbClient.query('create table if not exists prs ( id SERIAL PRIMARY KEY,channel VARCHAR (100) not null, pr VARCHAR (100) not null,reporter VARCHAR (100) not null,queued TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP);');
  ...
  pop: async (user = null, channel = null, data = null) => {
    ...
    try {
      ...
      const query = ['SELECT * FROM prs WHERE 1=1', ...queryParams, 'ORDER BY id LIMIT 1 FOR UPDATE'].join(' ');
