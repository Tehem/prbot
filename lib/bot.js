...
/**
 * explainScore translates the score in a human readable sentence.
 *
 * @param {number} score a number between 0 and 1
 * @returns {string} the human readable sentence
 */
function explainScore(score) {
  const asPercent = Math.round(score * 100);
  return `Your awesomeness score is ${asPercent}%.`;
}
...
async function answerPrSubmission(queue, message, config) {
  ...
  return `${config.name} in queue +${status.metadata.additions}/-${status.metadata.deletions}. ${explainScore(score)}`;
}
...
async function answerHelping(queue, message) {
  ...
  const score = await queue.getScore(message.user);
  return `${(pr && pr.pr) || 'Sorry, no more PRs for you in stock!'} ${explainScore(score)}`;
}
